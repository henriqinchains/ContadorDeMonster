...
function inicializarInterface(usuario) {
  ...
  // Injeção do avatar na Navbar
  const avatarNav = document.querySelector("#navUser .user-avatar");
  const avatarSalvo = localStorage.getItem(`avatar_${usuario.email}`);
  ...
  if (avatarNav) {
    if (avatarSalvo) {
      ...
      avatarNav.textContent = usuario.login
        ? usuario.login.substring(0, 2).toUpperCase()
        : "US";
    }
  }
  ...
  if (inputSujeito) {
    inputSujeito.value = usuario.login;
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (btnAplicar) {
    btnAplicar.addEventListener("click", () => {
      ...
      const notaMinima =
        parseFloat(document.getElementById("filtroNota").value) || 0;
      const ordem = document.getElementById("filtroOrdem").value;

      const tagAtiva = document.querySelector("#tagGroupValeu .tag.active");
      const valeuFiltro = tagAtiva
        ? tagAtiva.getAttribute("data-value")
        : "todos";
  ...
  if (btnLimpar) {
    btnLimpar.addEventListener("click", () => {
      ...
      document
        .querySelectorAll("#tagGroupValeu .tag")
        .forEach((t) => t.classList.remove("active"));
      const tagTodos = document.querySelector(
        "#tagGroupValeu .tag[data-value='todos']",
      );
...
async function deletarPost(id) {
  ...
  try {
    const resposta = await fetch(
      `https://monster-reviews-api.onrender.com/api/avaliacoes/${id}`,
      {
        method: "DELETE",
        credentials: "include",
      },
